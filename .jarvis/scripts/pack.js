...
// Slash command suffixes. Bare /jarvis is handled separately, with a guard so that
...
const PROJECT = [
  ...
  // Seeded empty; jarvis-staff appends to both from /jarvis-health onwards.
...
function templatize(text, rel) {
  ...
  // Slash commands, by exact name first, then bare /jarvis with a terminator guard.
  // An open-ended /jarvis rule would also rewrite `/jarvis-ignore` inside a regex
  // literal and `scripts/jarvis.js` inside a path.
  for (const c of COMMANDS) t = t.split(`/jarvis${c}`).join(`/{{name}}${c}`);
  t = t.replace(/\/jarvis(?![\w.-])/g, '/{{name}}');
  t = t.replace(/npm run (-s )?jarvis --/g, (m, s) => `npm run ${s || ''}{{name}} --`);
  return t;
}

function outPath(rel) {
  // .claude/agents/jarvis-po.md -> .claude/agents/{{name}}-po.md
