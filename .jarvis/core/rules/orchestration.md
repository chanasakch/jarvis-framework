...
Read by `/jarvis` in the main session. The orchestrator delegates; it never does phase work itself
...
3. Delegate with the Task tool using the **Handoff Brief** (§3.1).
   - **implement:** read `plan.md`, run tasks in `depends_on` order, **one task per subagent call**, routed by
     task `layer` (backend → `jarvis-dev-backend`, frontend → `jarvis-dev-frontend`). After each task run
   ...
   - **review:** invoke every reviewer in `agents_resolved` **in parallel — multiple Task calls in a single
     message**. That is `review.reviewers` plus `jarvis-review-devops` when `has_infra_change` is true.
...
4. **Agent gate:** invoke `jarvis-gatekeeper` with the artifacts, the phase checklist and the upstream artifacts.
...
| Intent | Command |
|---|---|
| Approve a `passed` phase | `! npm run -s jarvis -- approve <ID> <phase>` |
| Skip an optional phase | `! npm run -s jarvis -- skip <ID> <phase> --reason "<why>"` |
| Redo a phase and everything after it | `! npm run -s jarvis -- reopen <ID> <phase>` |
| Pause / resume an item | `! npm run -s jarvis -- park <ID> --reason "<why>"` / `unpark <ID>` |
