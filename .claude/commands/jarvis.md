...
Current status:
!`node .jarvis/scripts/jarvis.js status --brief 2>/dev/null || echo "jarvis: status unavailable (run /jarvis-init)"`
...
3. Never run `approve`, `force`, `skip`, `reopen`, `park` or `unpark`. `guard.js` blocks them.
   Print the command and ask the user to run it: `! npm run -s jarvis -- approve <ID> <phase>`.
