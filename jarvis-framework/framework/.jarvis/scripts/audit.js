...
// 2. workflow agents exist + tools match spec
{
  ...
  const SPEC_TOOLS={
    '{{name}}-gatekeeper':'Read, Grep, Glob','{{name}}-analyst':'Read, Write, Glob, Grep','{{name}}-po':'Read, Write, Glob, Grep',
    '{{name}}-ba':'Read, Write, Glob, Grep','{{name}}-ux':'Read, Write, Glob, Grep','{{name}}-investigator':'Read, Write, Glob, Grep, Bash',
    '{{name}}-architect':'Read, Write, Glob, Grep','{{name}}-planner':'Read, Write, Glob, Grep',
