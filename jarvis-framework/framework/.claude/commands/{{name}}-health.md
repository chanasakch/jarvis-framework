...
Run the codebase health review by delegating to `{{name}}-staff`. This is not a work item and has no
...
3. Delegate to `{{name}}-staff` with this brief, paths only:
...
5. For each proposed work item, print the command that would open it — do not run it:
   `! npm run -s {{name}} -- new <type> "<title>"`

If `{{name}}-staff` returns BLOCKED, print its `open_questions` and stop.
